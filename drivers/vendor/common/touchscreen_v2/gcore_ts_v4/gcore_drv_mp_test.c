...
int gcore_parse_mp_test_ini(struct gcore_mp_data *mp_data)
{
	...
	if (ret == 0) {
		...
		if (tmp) {
			...
			if (ret < 0)
				GTP_ERROR("read test-int-pin error.");
		}
		...
		if (tmp) {
			...
			if (ret < 0)
				GTP_ERROR("read test chip id error.");
		}
		...
		if (tmp) {
			...
			if (ret < 0)
				GTP_ERROR("read test open error.");
		}
		...
		if (tmp) {
			...
			if (ret < 0)
				GTP_ERROR("read test short error.");
		}
		...
		if (tmp) {
			...
			if (ret < 0)
				GTP_ERROR("read short min error.");
		}
...
#ifndef GCORE_IGNORE_MP_CMD_CHECK
...
#ifdef GKI_CHECK
int dump_mp_data_to_csv_file(const u16 *data, int rows, int cols, enum MP_DATA_TYPE types)
{
	int r = 0;
...
int dump_mp_info_to_csv_file(void)
{
...
int gcore_save_mp_data_to_file(struct gcore_mp_data *mp_data)
{
	...
	GTP_DEBUG("dump mp data to csv file, row: %d col: %d",rows, cols);

	ret = dump_mp_info_to_csv_file();
	if (ret < 0) {
	...
	if (mp_data->test_open) {
		ret = dump_mp_data_to_csv_file(mp_data->open_node_val, rows,
					       cols, OPEN_DATA);
		if (ret < 0) {
	...
	if (mp_data->test_short) {
		ret = dump_mp_data_to_csv_file(mp_data->short_node_val, rows,
					       cols, SHORT_DATA);
		if (ret < 0) {
	...
	if (mp_data->test_rawdata) {
		ret = dump_mp_data_to_csv_file(mp_data->rawdata_node_val, rows,
					     cols, RAW_DATA);
		if (ret < 0) {
...
#if MP_RAWDATA_DEC_TEST
		ret = dump_mp_data_to_csv_file(mp_data->rawdata_dec_h, rows,
					     cols, RAWDATA_DECH);
		if (ret < 0) {
		...
		ret = dump_mp_data_to_csv_file(filepath, mp_data->rawdata_dec_l, rows,
					     cols, RAWDATA_DECL);
		if (ret < 0) {
...
#endif
#if MP_RAWDATA_AVG_TEST
		ret = dump_mp_data_to_csv_file(mp_data->rawdata_avg_h, rows, cols);
		if (ret < 0) {
			GTP_ERROR("dump mp test rawdata avg h to file failed");
			goto fail;
		}

		ret = dump_mp_data_to_csv_file(mp_data->rawdata_avg_l, rows, cols);
		if (ret < 0) {
	...
	if (mp_data->test_noise) {
		ret = dump_mp_data_to_csv_file(mp_data->noise_node_val, rows,
						   cols, NOISE_DATA);
		if (ret < 0) {
...
int gcore_start_mp_test(void)
{
	...
	if (mp_data->test_chip_id) {
		...
		if (mp_data->chip_id_test_result) {
			gcore_tptest_result = gcore_tptest_result | TP_MODULE_TYPE_ERR;
	...
	if (mp_data->test_open || mp_data->test_short || mp_data->test_rawdata \
		...
		if ( ret != 0) {
			...
			if (retry_ito_test >= 3) {
				gcore_tptest_result = gcore_tptest_result | TP_OPEN_TEST_FAIL;
			}
			msleep(100);
            retry_ito_test = 0;
			...
			if (retry_ito_test >= 3) {
				gcore_tptest_result = gcore_tptest_result | TP_SHORT_TEST_FAIL;
			}
			msleep(100);
            retry_ito_test = 0;
            do{
			...
			if (retry_ito_test >= 3) {
				gcore_tptest_result = gcore_tptest_result | TP_RAWDATA_TEST_FAIL;
			}
			msleep(100);
            retry_ito_test = 0;
            do{
		   ...
		   if (retry_ito_test >= 3) {
				gcore_tptest_result = gcore_tptest_result | TP_NOISE_TEST_FAIL;
			}
		}
...
//	GTP_DEBUG("Result string head len %d.", len);
	...
	GTP_DEBUG("final result_buff len:%d.", result_buff_pos);
