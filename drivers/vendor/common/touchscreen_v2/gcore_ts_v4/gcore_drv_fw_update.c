...
int gcore_fw_event_notify(enum fw_event_type event)
{
	...
	switch (event) {
		...
		case FW_TEL_CALL:
			GTP_DEBUG("TEL gcore fw event: enable tp proximity");
			fw_event_cmd[9] = fw_event_cmd[9] | ((0x00 | 0x01) << 0);
			notify_byte = fw_event_cmd[9];
			break;

		case FW_TEL_HANDUP:
			GTP_DEBUG("TEL gcore fw event: disable tp proximity");
...
int check_notify_event(u8 rev_data,enum fw_event_type check_event)
{
	...
	if (tpd_cdev->bbat_test_enter) {
        if (tpd_cdev->bbat_int_test == false) {
            ...
            GTP_INFO("tpd int BBAT test success");
        }
        return IRQ_HANDLED;
    }
...
void add_CRC (u8 *fw_buf)
{
   ...
   if(CRCVal == 0) {
		...
		GTP_DEBUG("sCRC32Result = 0x%08X", sCRC32Result);
		// GTP_DEBUG("fw_buf[65528] = 0x%04X", fw_buf[65528]);
		// GTP_DEBUG("fw_buf[65529] = 0x%04X", fw_buf[65529]);
		// GTP_DEBUG("fw_buf[65530] = 0x%04X", fw_buf[65530]);
		// GTP_DEBUG("fw_buf[65531] = 0x%04X", fw_buf[65531]);
   }
...
void gcore_request_firmware_update_work(struct work_struct *work)
{
	...
	if (gcore_adb_upgrade_firmware) {
		if (gcore_adb_upgrade_firmware->size > FW_SIZE)
			return;
		memcpy(fw_update_buff, gcore_adb_upgrade_firmware->data, gcore_adb_upgrade_firmware->size);
		usleep_range(10000, 11000);
	} else {
		...
		if (request_firmware(&fw, FW_BIN_NAME, &gdev_fwu->bus_device->dev)) {
			...
			GTP_DEBUG(" try requeset %s", DEFAULT_UPDATE_FIRMWARE_NAME);
	...
	if (g_ret_update == 0) {
		GTP_DEBUG("firmware_update_work is running, ignore and return.");
		return;
	}
...
//	static u8 *fw_buf = NULL;
	...
	GTP_DEBUG("upgrade from file(%s) start!",fileName);
