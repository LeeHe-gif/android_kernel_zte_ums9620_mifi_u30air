...
int gcore_ts_check_default_tp(struct device_node *dt, const char *prop)
{
	...
	if (ret < 0) {
		GTP_ERROR("fail to read %s %d", prop, ret);
		ret = -ENODEV;
		goto out;
	}
...
void gcore_proximity_report_to_application(void){
	if(gcore_tpd_proximity_flag_off != gcore_tpd_proximity_flag_off_backup
		...
		if(gcore_tpd_proximity_flag_off){
			gcore_proxi_timer_start();
			GTP_DEBUG("tp proximity report: NEAR event");
		}else{
			GTP_DEBUG("tp proximity report: FAR event");
		}
		input_report_abs(gdev_intf->proximity_input_dev, ABS_DISTANCE, tpd_cdev->tpd_proximity_state);
...
#ifdef HUB_TP_PS_ENABLE
	if(tpd_cdev->tpd_proximity_enable){
		if(gdev->TEL_State){
		...
		if(coor_data[62] & 0x01){
			...
			if((coor_data[62] == 0x81) && tpd_cdev->tpd_proximity_state && !tpd_cdev->tpd_proximity_suspended){
				gdev->tel_screen_off = true;
				gcore_tpd_proximity_flag_off = true;
				tpd_cdev->tpd_proximity_state = 0;
				GTP_DEBUG("tp proximity report: NEAR event, data[62]=0x%x",coor_data[62]);
				input_report_abs(gdev->proximity_input_dev, ABS_DISTANCE, 0);
				input_sync(gdev->proximity_input_dev);
			}else if((coor_data[62] == 0x01) && tpd_cdev->tpd_proximity_suspended){
				gdev->tel_screen_off = false;
				gcore_tpd_proximity_flag_off = false;
				tpd_cdev->tpd_proximity_suspended = false;
				GTP_DEBUG("tp proximity report: FAR event,data[62]=0x%x",coor_data[62]);
				input_report_abs(gdev->proximity_input_dev, ABS_DISTANCE, 1);
				input_sync(gdev->proximity_input_dev);
			}/* else {
				GTP_DEBUG("tpd_proximity_state data[62]=0x%x",coor_data[62]);
			} */
			/* gcore_proximity_report_to_application(); */
		}
	}
...
#ifdef HUB_TP_PS_ENABLE
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 6, 0))
	device_init_wakeup(&gdev->bus_device->dev, true);
	GTP_DEBUG("device_init_wakeup true.");
...
#ifdef HUB_TP_PS_ENABLE
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 6, 0))
		device_init_wakeup(&gdev->bus_device->dev, false);
		GTP_DEBUG("device_init_wakeup false.");
#elif defined(CONFIG_PM_WAKELOCKS)
		wakeup_source_unregister(gdev->prx_wake_lock_ps);
#else
		wake_lock_destroy(&gdev->prx_wake_lock_ps);
#endif
#endif
		return -ENOMEM;
	}
...
#endif
#endif
#ifdef CONFIG_TOUCH_DRIVER_INTERFACE_SPI_MODE
	gcore_spi_slave->mode = SPI_MODE_3;
	GTP_DEBUG("tpd_spi_mode is SPI_MODE_3.");
#else
