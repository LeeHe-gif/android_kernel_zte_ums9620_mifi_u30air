		...
		const char *buf, size_t count)
{
	...
	if (ret != 2) {
		GTP_ERROR("sscanf tp_ps enable data error!!! ret = %d", ret);
		return count;
	}

	if (!gdev_intf->proximity_input_dev) {
		GTP_ERROR("enable psensor fail : have no input dev");
		return count;
	}

	mutex_lock(&gdev_intf->proximity_input_dev->mutex);
	enable = (enable > 0) ? 1 : 0;
	tpd_cdev->tpd_proximity_enable = enable;
	tpd_cdev->tpd_proximity_suspended = false;
	tpd_cdev->tpd_proximity_state = 1;

	do {
		ret = tpd_enable_ps(enable);
		i++;
		if (ret == 0) {
			GTP_INFO("set tp proximity %s success!", enable ? "enable" : "disable");
			break;
		} else {
			msleep(100);
			GTP_INFO("Failed to set tp proximity command ,retry times %d", i);
		}
	} while (i < 3);

	/* init value far for vts test*/
	if (enable) {
		input_report_abs(gdev_intf->proximity_input_dev, ABS_DISTANCE, 1);
		input_sync(gdev_intf->proximity_input_dev);
	}

	mutex_unlock(&gdev_intf->proximity_input_dev->mutex);
	return count;
}
		...
		const char *buf, size_t count)
{
	...
	if (flush_count % 2 == 0) {
		...
		input_report_abs(gdev_intf->proximity_input_dev, ABS_DISTANCE, -2);
		flush_count = 0;
	}
	input_sync(gdev_intf->proximity_input_dev);
...
static int tp_ps_init(void)
{
	...
	GTP_INFO("enter!");
	tpd_cdev->tpd_proximity_state = 1;
	tpd_cdev->tpd_proximity_enable = 0;
	tpd_cdev->tpd_proximity_suspended = false;
	/* allocate proximity input_device */
	gdev_intf->proximity_input_dev = input_allocate_device();
	if (gdev_intf->proximity_input_dev == NULL) {
		GTP_ERROR("could not allocate proximity input device");
		return -ENODEV;
	}

	input_set_drvdata(gdev_intf->proximity_input_dev, gdev_intf);
	gdev_intf->proximity_input_dev->name = TP_PS_INPUT_DEV;
	gdev_intf->proximity_input_dev->phys = TP_PS_INPUT_DEV;
	input_set_capability(gdev_intf->proximity_input_dev, EV_ABS, ABS_DISTANCE);
	input_set_abs_params(gdev_intf->proximity_input_dev, ABS_DISTANCE, 0, 1, 0, 0);

	/*add class sysfs for tp_ps*/

	err = class_register(&ps_sensor_class);
	if (err < 0) {
		GTP_ERROR("Create fsys class failed (%d)", err);
		goto err_class_create;
	}

	err = class_create_file(&ps_sensor_class, &class_attr_delay);
	if (err < 0) {
		GTP_ERROR("Create delay file failed (%d)", err);
		goto exit_unregister_class;
	}

	err = class_create_file(&ps_sensor_class, &class_attr_enable);
	if (err < 0) {
		GTP_ERROR("Create enable file failed (%d)", err);
		goto exit_unregister_class;
	}

	err = class_create_file(&ps_sensor_class, &class_attr_flush);
	if (err < 0) {
		GTP_ERROR("Create flush file failed (%d)", err);
		goto exit_unregister_class;
	}

	err = class_create_file(&ps_sensor_class, &class_attr_batch);
	if (err < 0) {
		GTP_ERROR("Create batch file failed (%d)", err);
		goto exit_unregister_class;
	}

	err = input_register_device(gdev_intf->proximity_input_dev);
	if (err < 0) {
		GTP_ERROR("Could not register psensor input device");
...
exit_unregister_class:
	GTP_ERROR("unregister tp_ps_sensor_class.");
	class_unregister(&ps_sensor_class);
err_class_create:
	input_free_device(gdev_intf->proximity_input_dev);
	return -1;
}

static int tp_ps_deinit(void)
{
	GTP_INFO("unregister tp_ps_sensor_class.");
	class_unregister(&ps_sensor_class);
	input_free_device(gdev_intf->proximity_input_dev);
	return 0;
}
